...
1. **Open PrivateVoice.** The app starts a local TTS server on port 8765.
...
**Note:** MP3 export has a known issue in v1.0 and may not work correctly. WAV export is reliable.
...
PrivateVoice includes built-in speech-to-text using Whisper models. This can transcribe audio files back to text.

### Loading a Whisper Model

The Whisper model must be loaded separately from the TTS model.
...
2. Load a model via the API: `POST /load-whisper` with `{"model_size": "tiny"}`
3. The model runs on CPU (does not compete with GPU memory used by TTS models).

### Transcribing Audio

Send an audio file to the transcription endpoint:
- `POST /transcribe` with multipart form data containing the audio file.
- Returns: transcribed text, detected language, confidence score (0-1), and audio duration.

**Note:** The Whisper transcription feature may show as "Coming Soon" in the desktop UI. It is fully functional via the API.
...
The app supports keyboard navigation for accessibility. Specific shortcuts are documented in the Help panel within the app.
...
### Not enough memory
The memory check endpoint reports whether you have enough free RAM for a given model. Close other applications to free memory. The 0.6B models need ~8GB free; 1.7B models need ~12GB.
...
### Audio export fails
If MP3 export fails, switch to WAV format in Settings. WAV export is more reliable.
